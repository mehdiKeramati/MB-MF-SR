...
function negLogLikelihood = negLogLikelihood ( parameterValues , data , componentMB , componentSR , componentMF , environmentalParameters );
    ...
    if componentMB  eMB = exp(parameterValues(1));  else    eMB = 0;    end    
    if componentSR  eSR = exp(parameterValues(2));  else    eSR = 0;    end    
    ...
    w_MB                            = eMB / normalizer                              ;
    w_SR                            = eSR / normalizer                              ;    
    if componentMF             w_MF = 1 - w_MB - w_SR ;   else w_MF=0;    end       ;    
    beta                            =             exp( parameterValues( 3 ) )       ; 
    TransitionLearningRate          = 1.0 / ( 1 + exp(-parameterValues( 4 )))       ;
    TransitionDriftRate             = 1.0 / ( 1 + exp(-parameterValues( 5 )))       ;
    RewardLearningRate              = 1.0 / ( 1 + exp(-parameterValues( 6 )))       ;
    RewardDriftRate                 = 1.0 / ( 1 + exp(-parameterValues( 7 )))       ;
    OccupancyLearningRate           = 1.0 / ( 1 + exp(-parameterValues( 8 )))       ;
    OccupancyDriftRate              = 1.0 / ( 1 + exp(-parameterValues( 9 )))       ;
    OccupancyEligibilityTrace       = 1.0 / ( 1 + exp(-parameterValues( 10)))       ;    
    MFValueLearningRate             = 1.0 / ( 1 + exp(-parameterValues( 11)))       ;
    MFValueDriftRate                = 1.0 / ( 1 + exp(-parameterValues( 12)))       ;        
    MFEligibilityTrace              = 1.0 / ( 1 + exp(-parameterValues( 13)))       ;    
    choiceStayBias                  =                  parameterValues( 14 )        ;
    motorStayBias                   =                  parameterValues( 15 )        ;    
